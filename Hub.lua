...
lib.create_window = function(theme, menu_key)
	...
	UIGridLayout.CellSize = UDim2.new(0, 90, 0, 70) -- Увеличен размер влево
	...
	window.create_tab = function(image)
		...
		tab.create_sector = function(name)
			...
			Sector.Size = UDim2.new(0.5, -5, 1, 0) -- Изменено для возможности создания 2 панелей
			...
			sector.dropdown = function(text, options, default, callback)
				...
				UIPadding.PaddingLeft = UDim.new(0, 5) -- Изменено для стрелочки справа
			...
			sector.toggle = function(text,default,callback)
				...
				Text.BorderColor3 = Color3.fromRGB(0, 0, 0)
				Text.BorderSizePixel = 0
				Text.Position = UDim2.new(0.0923694745, 0, 0, 0) -- Текст чекбокса
				...
				toggle.add_color = function(_default, cpcallback)
					...
					ColorPicker.Size = UDim2.new(0, 150, 0, 80) -- Изменен размер колорпикера (X больше, Y меньше)
					...
					Saturation.Size = UDim2.new(0, 76, 0, 56) -- Изменен размер сатурации
					...
					Hue.Size = UDim2.new(0, 13, 0, 56) -- Изменен размер hue
			...
			sector.slider = function(text, indicator, min, max, default, callback)
				...
				Text.BorderColor3 = Color3.fromRGB(0, 0, 0)
				Text.BorderSizePixel = 0
				Text.Position = UDim2.new(0.0923694745, 0, -0.0476190485, 0) -- Текст слайдера на той же позиции что и у чекбокса
